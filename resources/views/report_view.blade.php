    ...
    <div class="flex md:flex-row flex-col items-center md:justify-between w-full">
        <div class="flex flex-wrap md:flex-nowrap items-center w-full md:w-auto mb-3 space-x-0 md:space-x-3">
            ...
            <h2 class="text-4xl font-semibold bg-gradient-to-b from-[#1F5077] to-[#3A95DD] bg-clip-text text-transparent">
                {{ $report->branch->branch_name ?? 'No Branch Assigned' }}
            </h2>
        </div>
    ...
    <div class="flex md:flex-row flex-col md:justify-between bg-[#F1FAFE] rounded-[20px] shadow-sm p-3 my-4">
        ...
        <div class="md:w-1/2 w-full md:order-1 order-2">
            <div class="flex md:mb-3 mb-2">
                ...
                <div class="w-1/2">
                    <p class="text-sm text-[#3A95DD]">{{ $report->branch_location ?? 'N/A' }}</p>
                </div>
            </div>
            ...
            <div class="flex md:mb-0 mb-2">
                ...
                <div class="w-1/2">
                    <p class="text-sm text-[#3A95DD]">{{ $report->created_at ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="md:w-1/2 w-full md:order-2 order-3">
            ...
            <div class="flex md:mb-0 mb-2">
                ...
                <div class="w-1/2">
                    <p class="text-sm text-[#3A95DD]">{{ $report->user->name ?? 'Unknown User' }}</p>
                </div>
            </div>
        </div>
        ...
        @foreach($report->sections as $section)
            <div class="">
                ...
                @if($section->questions && count($section->questions) > 0)
                            ...
                            <div class="ml-10">
                                ...
                                <div class="flex flex-col bg-black/[2%] border border-[#D9D9D9] rounded-lg p-4 w-full">
                                    ...
                                    <div class="flex items-center mb-4">
                                        ...
                                        <div class="ml-3">
                                            <p class="font-semibold text-[#1D3F5D]">{{ $report->user->name ?? 'Unknown User' }}</p>
                                            <p class="text-sm text-gray-500">{{ $report->created_at ?? 'N/A' }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex lg:flex-row flex-col md:ml-16">
                                        ...
                                        <div class="grid md:grid-cols-6 grid-cols-3 w-full md:gap-4 gap-2">
                                            @if(count($attachments) > 0)
                                                ...
                                                <a href="{{ url('/' . $img) }}" target="_blank"><img class="h-20 w-20 rounded-md object-cover" src="{{ url('/' . $img) }}" alt="Attachment {{ $imgKey + 1 }}" /></a>
                                                @endforeach
                                            @else
                    ...
                    @endforeach
                @else
                    <p class="text-sm text-gray-500">No questions found for this section.</p>
                @endif
            </div>
        @endforeach
    @else
        <p class="text-sm text-gray-500">No sections found for this report.</p>
    @endif
</div>
